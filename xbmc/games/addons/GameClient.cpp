...
bool CGameClient::Serialize(uint8_t* data, size_t size)
{
  ...
  if (m_bIsPlaying)
  {
    ...
    try
    {
      bSuccess = LogError(m_struct.toAddon.Serialize(buffer.data, buffer.size), "Serialize()");
    }
    catch (...)
    {
...
bool CGameClient::Deserialize(const uint8_t* data, size_t size)
{
  ...
  if (m_bIsPlaying)
  {
    ...
    try
    {
      bSuccess = LogError(m_struct.toAddon.Deserialize(packet.data, packet.size), "Deserialize()");
    }
    catch (...)
    {
