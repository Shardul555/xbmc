...
CGUIControllerPanel::CGUIControllerPanel(int parentID,
                                         int controlID,
                                         float posX,
                                         float posY,
                                         float width,
...
CGUIControllerPanel::CGUIControllerPanel(const CGUIControllerPanel& other)
  : CGUIPanelContainer(other), m_grid(new CControllerGrid(*other.m_grid))
{
  InitializeBaseContainer();
}
...
{
  CGUIControllerProvider* controllerProvider =
      dynamic_cast<CGUIControllerProvider*>(m_listProvider);
...
unsigned int CGUIControllerPanel::GetItemSize() const
{
  if (m_grid)
  {
    ...
    if (height > 0)
      return static_cast<unsigned int>(m_height) / height;
  }
