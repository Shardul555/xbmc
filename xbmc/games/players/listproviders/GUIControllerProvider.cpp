...
CGUIControllerProvider::CGUIControllerProvider(int parentID) : IListProvider(parentID)
{
}
...
{
  CGUIControllerItem* listItem = dynamic_cast<CGUIControllerItem*>(item.get());
...
bool CGUIControllerProvider::OnInfo(const CGUIListItemPtr& item)
{
  return OnClick(item);
}

bool CGUIControllerProvider::OnContextMenu(const CGUIListItemPtr& item)
{
  return OnClick(item);
}
...
void CGUIControllerProvider::SetGrid(const CControllerGrid& grid)
{
  ...
  for (const auto& column : grid.Grid())
  {
    ...
    {
      if (row.bVisible)
        m_items.emplace_back(new CGUIControllerItem(row.controller));
      else
        m_items.emplace_back(new CGUIListItem);
    }
  }
